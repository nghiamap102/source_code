...
import SocialRegister from 'src/views/Login/SocialForm'
import ListType from 'src/views/Login/ListType'
...
const SocialRegisterWrapper = ({ countries }: any) => {
  const [step, setStep] = useState<number>(1)
  const { isOpen, onOpen, onClose } = useDisclosure()
  const { dataRegisterSocial, errorMessage, successMessage, urlReturn }: any = useSelector<RootReducer>(
    (item) => item.auth
  )
  ...
  const handleChangeCuisineType = (value: stringOrNumber[]) => {
    dispatch(updateDataRegister({ ...dataRegisterSocial, cuisine_types: value }))
  }

  const handleChangeCookingLevel = (value: stringOrNumber[]) => {
    dispatch(updateDataRegister({ ...dataRegisterSocial, cooking_level: value }))
  }

  const handleChangeSpecialTags = (value: stringOrNumber[]) => {
    dispatch(updateDataRegister({ ...dataRegisterSocial, special_tags: value }))
  }

  const handleChangeKeyIngredients = (value: stringOrNumber[]) => {
    dispatch(updateDataRegister({ ...dataRegisterSocial, key_ingredients: value }))
  }
  ...
  const arrRenderUI = [
    <SocialRegister
      setStep={setStep}
      dataSocialRegister={dataRegisterSocial}
